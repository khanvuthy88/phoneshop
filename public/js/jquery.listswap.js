(function($) {
    $.fn.listswap = function(options) {
        ...
        var controls = '<div class="listbox_controls">' +
            '<ul>' +
            '<li class="addall btn btn-primary"><i class="fa fa-angle-double-right"></i></li>' +
            '<li class="add btn btn-primary"><i class="fa fa-angle-right"></i></li>' +
            '<li class="remove btn btn-primary"><i class="fa fa-angle-left"></i></li>' +
            '<li class="removeall btn btn-primary"><i class="fa fa-angle-double-left"></i></li>' +
            '</ul>' +
            '</div>';
        ...
        function add_remove_handler(target, destination, select_1, select_2) {
            ...
            if(select_1 == "destination_3"){
                $('.permission-'+data_value).remove();
                $("#storeCheckbox").append($('<input type="checkbox" name="permission[]"/>').attr("class", "permission-"+data_value).attr("value",data_value).text(text));
            }else {

                $("#storeCheckbox").append($('<input type="checkbox" name="permission[]" checked />').attr("value",data_value).attr("class", "permission-"+data_value).text(text));
            }
            target.remove();
        }
