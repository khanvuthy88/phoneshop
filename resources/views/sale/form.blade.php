                        ...
                        <div class="input-group">
                            <select name="client" id="client" class="form-control select2" required>
                                @foreach ($clients as $client)
                                    <option value="{{ $client->id }}" {{ selectedOption($client->id, old('client')) }} @if($client->is_default) selected @endif>
                                        {{ $client->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-lg-4 form-group">
                            ...
                            <select name="branch" id="branch" class="form-control select2" required>
                                ...
                                @foreach ($branches as $branch)
                                    <option value="{{ $branch->id }}" {{ selectedOption($branch->id, old('branch')) }}>
                                        {{ $branch->location }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-lg-4 form-group">
                            ...
                            <select name="agent" id="agent" class="form-control select2" required>
                                ...
                                @foreach ($agents as $agent)
                                    <option value="{{ $agent->id }}" {{ selectedOption($agent->id, old('agent')) }}>
                                        {{ $agent->name }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="row">
                    ...
                    <div class="col-lg-4 form-group">
                        ...
                        <input type="text" name="sale_date" id="sale_date" class="form-control date-picker" required
                               placeholder="{{ trans('app.date_placeholder') }}" value="{{ old('sale_date') ?? date('d-m-Y') }}">
                    </div>
                    <div class="col-lg-4 form-group">
                        <label for="sale_date" class="control-label">
                        ...
                        <select name="status" id="status" class="form-control" required>
                            ...
                            @foreach (saleStatuses() as $k => $_sta)
                                <option value="{{ $k }}" {{ selectedOption($k, old('status')) }}>
                                    {{ $_sta }}
                                </option>
                            @endforeach
                        </select>
